...
sizes.NumOutputs     = 6;
sizes.NumInputs      = 16;%%19 16 in gp
...
%     MPCParameters.Nc  = 5; %  Tsplit
    MPCParameters.Ts  = 0.07; % the sample time of near term  
...
function sys = mdlOutputs(t,x,u)
   ...
   %输入接口转换,x_dot后面加一个非常小的数，是防止出现分母为零的情况，这与我们建立的状态量的顺序一致。《第一版p111//第二版p117》
...
%     Y_L2 = u(14); %左后轮y坐标
%     Y_R2 = u(15); %右后轮y坐标
%     Vx_L2 = u(16)/3.6+0.0001;  %左后轮纵向速度，Unit:km/h
%     Vx_R2 = u(17)/3.6+0.0001; %右后轮纵向速度，Unit:km/h
...
%     X =  0.5*(X_L2 + X_R2);%后轴中心X坐标，Unit:m
%     Y = 0.5*(Y_L2 + Y_R2);%后轴中心Y坐标，Unit:m
%     x_dot = 0.5*(Vx_L2 + Vx_R2);%后轴中心处纵向速度,Unit：m/s
% %% 车辆参数输入
...
%     load path150 %%129--198km/h
    ...
    %自主车辆线性时变模型预测路径跟踪控制
    state_k1(1,1)=y_dot+T*(-x_dot*phi_dot+2*(Ccf*(delta_f-(y_dot+Lf*phi_dot)/x_dot)+Ccr*(Lr*phi_dot-y_dot)/x_dot)/m)-mean_vy;
    state_k1(2,1)=x_dot+T*(y_dot*phi_dot+2*(Clf*Sf+Clr*Sr+Ccf*delta_f*(delta_f-(y_dot+phi_dot*Lf)/x_dot))/m)-mean_vx;
    state_k1(3,1)=phi+T*phi_dot;
    state_k1(4,1)=phi_dot+T*((2*Lf*Ccf*(delta_f-(y_dot+Lf*phi_dot)/x_dot)-2*Lr*Ccr*(Lr*phi_dot-y_dot)/x_dot)/I)-mean_yaw_rate;
...
%     b_cons=cell2mat(b_cons_cell);%（求解方程）状态量不等式约束的取值
    ...
    ref_y = path5(:,2);
    ref_yaw = path5(:,3);
    ref_yaw_rate = path5(:,4);
    ref_vy = path5(:,5);
    
    out_y_ref = interp1(pt,ref_y,t);
    out_x_ref = interp1(pt,ref_x,t);
    out_yaw_ref = interp1(pt,ref_yaw,t);
    out_yaw_rate_ref = interp1(pt,ref_yaw_rate,t);
    out_vy_ref = interp1(pt,ref_vy,t);
...
%     x_start=zeros(Nc+1,1);%加入一个起始点
    ...
    OUT(2) = out_y_ref;
    OUT(3) = out_x_ref;
    OUT(4) = out_yaw_ref*180/3.14;
    OUT(5) = out_yaw_rate_ref*180/3.14*3.0476; %36-55
%     OUT(5) = out_yaw_rate_ref*180/3.14;
    OUT(6) = out_vy_ref/3.6;
%     OUT(6) = out_vy_ref;
