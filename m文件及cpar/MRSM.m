...
if exist( 'option', 'var' )
    ...
    if isempty(option.s)              % Upper bound for matrix A elements. 
                                    %Setting this parameter to a very small value means we do not consider any correlation amng the responses
        model.s=0.6;               
    else
        model.s=option.s;
    end
